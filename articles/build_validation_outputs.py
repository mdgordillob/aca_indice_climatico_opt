...
def build_lowess_bands(aci_full: dict, frac: float = 0.15):
    """
    LOWESS smoother (~10-yr bandwidth) + ±1.5 sigma residual envelope.
    frac=0.15 on a 768-month series gives ~115-month (~9.6-yr) local window.
    Returns the LOWESS smooth as 'med' and constant-width ±0.75 / ±1.5 sigma
    bands as p25/p75 and p05/p95.  'obs' carries the raw monthly composite so
    the caller can plot monthly bars behind the smooth.
    """
    ...
    valid = ~np.isnan(obs_composite)

    pts    = sm_lowess(obs_composite[valid], t_arr[valid], frac=frac, return_sorted=True)
    smooth = np.interp(t_arr, pts[:, 0], pts[:, 1])

    resid = obs_composite - smooth
    sigma = float(np.nanstd(resid))

    p05 = smooth - 1.5 * sigma
    p25 = smooth - 0.75 * sigma
    p75 = smooth + 0.75 * sigma
    p95 = smooth + 1.5 * sigma

    return full.index, obs_composite, smooth, p05, p25, p75, p95, sigma


# keep old name as alias so Table-6a loop still works
def build_bootstrap(aci_full: dict, **kwargs):
    idx, obs, smooth, p05, p25, p75, p95, _ = build_lowess_bands(aci_full)
    return idx, obs, smooth, p05, p25, p75, p95


def fig8a_bootstrap(aci_full: dict):
    print("  Building LOWESS trend + residual envelope...")
    idx, obs_mo, smooth, p05, p25, p75, p95, sigma = build_lowess_bands(aci_full)

    fig, ax = plt.subplots(figsize=(11, 5))

    # Monthly bars in background
    ax.bar(idx, obs_mo, width=28, color="#3c7ebf", alpha=0.22, linewidth=0,
           label="Monthly ACI-CO")

    # ±1.5σ outer envelope
    ax.fill_between(idx, p05, p95, alpha=0.22, color="#2166ac",
                    label=f"±1.5σ residual envelope (σ={sigma:.2f})")
    # ±0.75σ inner envelope
    ax.fill_between(idx, p25, p75, alpha=0.38, color="#2166ac",
                    label="±0.75σ residual envelope")

    # LOWESS trend
    ax.plot(idx, smooth, color="#08306b", linewidth=2.4, label="LOWESS trend (~10-yr bandwidth)")

    # 5-yr MA for comparison
    ma5 = pd.Series(obs_mo, index=idx).rolling(60, min_periods=36).mean()
    ax.plot(ma5.index, ma5.values, color="black", linewidth=1.4,
            linestyle="--", alpha=0.70, label="5-yr moving average")

    ax.axvline(pd.Timestamp("1990-12-01"), color="gray", linestyle=":", linewidth=1)
    ax.axhline(0, color="black", linewidth=0.5)
    ax.set_ylabel("ACI-CO (σ)")
    ax.set_xlabel("Year")
    ax.set_title(
        "National ACI-CO — LOWESS trend with ±1.5σ residual envelope (1961–2024)",
        fontsize=12, fontweight="bold")
    ...
    print(f"  LOWESS residual sigma = {sigma:.3f}  |  +/-1.5 band width = {3*sigma:.3f}")

    # Save CSV so fig4 can overlay bands (p05/p25/p75/p95 = LOWESS ±σ bands)
    boot_df = pd.DataFrame({
        "date": idx, "obs_ma": smooth, "p05": p05, "p25": p25,
        "p75": p75, "p95": p95,
    })
