...
PROC = os.path.join(BASE, r'data\processed')
RAW  = os.path.join(BASE, r'data\raw')
...
def load_enso_data():
    try:
        sst = pd.read_csv(os.path.join(PROC, 'sst_index_colombia_pacific.csv'), parse_dates=['time'])
        oni_file = os.path.join(PROC, 'oni.ascii.txt') if os.path.exists(os.path.join(PROC, 'oni.ascii.txt')) else os.path.join(RAW, 'oni.ascii.txt')
        oni_raw = pd.read_csv(oni_file, sep=r'\s+', engine='python')
...
def load_tuning_data():
    try:
        flags = pd.read_csv(os.path.join(PROC, 'enso_flag_comparison.csv'), parse_dates=['time'])
        tuning = pd.read_csv(os.path.join(PROC, 'threshold_search_results.csv'))
        return flags, tuning
...
def _read_and_clean_csv(filepath, cols_to_keep):
    ...
    if {'Año', 'Mes'}.issubset(df.columns): df.rename(columns={'Año': 'year', 'Mes': 'month'}, inplace=True)
    if not {'year', 'month'}.issubset(df.columns): return pd.DataFrame()
...
def load_regional_ica_data(region_key):
    ...
    region_dir = os.path.join(PROC, folder_name)

    df_temp = _read_and_clean_csv(os.path.join(region_dir, 'anomalies_temperature_combined.csv'), ['t_90', 't_10'])
    df_rain = _read_and_clean_csv(os.path.join(region_dir, 'anomalies_precipitation_combined.csv'), ['Anomalia_Lluvia'])
    df_drought = _read_and_clean_csv(os.path.join(region_dir, 'anomalies_drought_combined.csv'), ['Anomalia_Sequia'])
    df_wind = _read_and_clean_csv(os.path.join(region_dir, 'anomalies_wind_combined.csv'),['anomalies_above'])
...
with st.sidebar:
    ...
    with st.container(border=True):
        map_data = pd.DataFrame([REGION_COORDS.get(selected_region, REGION_COORDS["Colombia"])])
        fig_map = px.scatter_mapbox(map_data, lat="lat", lon="lon", zoom=map_data["zoom"].iloc[0],
                                    mapbox_style="carto-positron", height=250)
    ...
    if not df_reg.empty:
        ...
        if 'ICA' in df_reg.columns:
            with st.container(border=True):
                ...
                st.plotly_chart(apply_sleek_theme(fig_ica), use_container_width=True)
        ...
        if {'t_90', 't_10'}.issubset(df_reg.columns):
            with st.container(border=True):
                ...
                st.plotly_chart(apply_sleek_theme(fig_temp), use_container_width=True)
        ...
        with c1:
            ...
            if fig_rain:
                with st.container(border=True): st.plotly_chart(fig_rain, use_container_width=True)
                
            fig_wind = create_bar_line_plot(df_reg, 'anomalies_above', 'anomalies_above_5yr_avg', "Wind Power Anomalies")
            if fig_wind:
                with st.container(border=True): st.plotly_chart(fig_wind, use_container_width=True)

        with c2:
            fig_drought = create_bar_line_plot(df_reg, 'Anomalia_Sequia', 'Anomalia_Sequia_5yr_avg', "Consecutive Dry Days Anomalies")
            if fig_drought:
                with st.container(border=True): st.plotly_chart(fig_drought, use_container_width=True)
            ...
            with st.container(border=True):
                ...
                if available_vars:
                    ...
                    st.plotly_chart(apply_sleek_theme(fig_corr), use_container_width=True)
...
with t2:
    if not df_enso.empty:
        with st.container(border=True):
            ...
            st.plotly_chart(apply_sleek_theme(fig1), use_container_width=True)
        ...
        with c1:
            with st.container(border=True):
                ...
                st.plotly_chart(apply_sleek_theme(fig2), use_container_width=True)
        ...
        with c2:
            with st.container(border=True):
                ...
                st.plotly_chart(apply_sleek_theme(fig3), use_container_width=True)
    ...
    if not df_enso.empty:
        ...
        with c1:
            with st.container(border=True):
                ...
                if os.path.exists(os.path.join(PROC, 'signal_geometry.png')): st.image(Image.open(os.path.join(PROC, 'signal_geometry.png')))
        with c2:
            with st.container(border=True):
                st.markdown("**Grid Correlation Analysis (GRIB Data)**")
                if os.path.exists(os.path.join(PROC, 'signal_spatial_corr.png')): st.image(Image.open(os.path.join(PROC, 'signal_spatial_corr.png')))
        ...
        with st.container(border=True):
            ...
            st.plotly_chart(apply_sleek_theme(fig_div), use_container_width=True)
...
with t4:
    ...
    if flags is not None and tuning is not None:
        ...
        with st.container(border=True):
            ...
            st.plotly_chart(apply_sleek_theme(fig_time), use_container_width=True)
        ...
        with c1:
            with st.container(border=True):
                ...
                st.plotly_chart(apply_sleek_theme(fig_hm), use_container_width=True)
                
        with c2:
            with st.container(border=True):
                st.markdown("#### Classification Metrics (Before vs. After Tuning)")
                if os.path.exists(os.path.join(PROC, 'confusion_matrix.png')):
                    st.image(Image.open(os.path.join(PROC, 'confusion_matrix.png')))
    else:
